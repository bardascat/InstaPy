...
class LikeForLike:
    ...
    def start(self):
        ...
        totalToLikeResult = api_db.fetchOne(sqlTotal,
                                            self.campaign['id_user'], self.campaign['id_user'],
                                            self.campaign['id_user'])
        ...
        while havePendingWork is True and securityBreak > iteration:
            ...
            post = api_db.fetchOne(sqlForGettingOnePost, self.campaign['id_user'], self.campaign['id_user'],
                                   self.campaign['id_user'])
            if post is None:
            ...
            if havePendingWork is True:
                pause = randint(1, 1)
                self.logger.info("LikeForLike:performLikes:: Going to sleep %s seconds until proceeding to next post",
                                 pause)
                time.sleep(pause)
